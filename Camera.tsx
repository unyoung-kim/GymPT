...
import { WorkoutCard } from "./workoutCount.card";
...
export default function CameraView({ changeRep, inWorkout, workout }) {
  ...
  useEffect(() => {
    ...
    if (poses != null && (poses.length > 0) & inWorkout) {
      if (workout == "shoudler press") {
        ...
        p2_y = poses[0].keypoints[14].y;
      }

      if (p1_y < p2_y) {
        setStatusDown(true);
      } else if (p1_y > p2_y && statusDown) {
        setStatusDown(false);
        setCount(count + 1);
        changeRep();
      }
...
const styles = StyleSheet.create({
  containerPortrait: {
    ...
    height: CAM_PREVIEW_HEIGHT, //CAM_PREVIEW_HEIGHT,
